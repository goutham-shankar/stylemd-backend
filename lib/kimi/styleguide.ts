 ...
 */
export async function runKimiStyleguideQuery(input: KimiStyleguideQueryInput): Promise<string> {
  const {
    workspaceDir,
    systemPrompt,
    prompt,
  ...
  const maxRetries = 2;
  const retryDelayMs = 15000; // longer default delay for styleguide
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (isRateLimit) {
        if (isLastAttempt) {
          console.warn(`\u26A0\uFE0F  [KIMI] STYLEGUIDE/SHOWCASE stage rate-limited after ${attempt + 1} attempts; styleguide fallback may be applied by orchestrator`);
        } else {
      ...
      throw new Error(
        `Kimi styleguide query failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
