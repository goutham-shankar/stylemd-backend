...
// ---------------------------------------------------------------------------
// Types matching semantic_analysis.json produced by Stage 2 (EXTRACT)
// ---------------------------------------------------------------------------
...
function isSimpleRadius(val: string): boolean {
...
function assignColorName(c: ObservedColor, index: number, primaryHex: string): string {
  ...
  if (h === "#ffffff") return "surface";
  if (h === "#000000") return "ink";
  if (c.areaWeight > 20 && isBackground && !isText) return "canvas";
  if (c.areaWeight > 5 && isBackground) return "canvas-soft";
  if (isText && !isBackground && c.frequency > 50) return "foreground";
  if (isText && !isBackground && h.includes("ee") && h.includes("00")) return "link";
  if (isBackground && c.areaWeight > 1) return `surface-${index}`;
  if (isText) return `text-${index}`;
  return `accent-${index}`;
}

function colorRole(c: ObservedColor): string {
  const roles = c.roles.map((r) => r.replace("_", " "));
  if (c.areaWeight > 30) return `Dominant — ${roles.join(", ")}`;
  if (c.areaWeight > 5) return `Major — ${roles.join(", ")}`;
  if (c.areaWeight > 1) return `Supporting — ${roles.join(", ")}`;
...
function typoRoleName(scale: TypographyScale, index: number): string {
  const size = pxToNum(scale.fontSize);
  const weight = parseInt(scale.fontWeight, 10);
  if (size >= 28 && weight >= 500) return "display";
  if (size >= 20 && weight >= 500) return "heading";
  if (size >= 16 && weight >= 600) return "ui-label";
  if (size >= 16 && scale.tag === "a") return "link";
  if (size >= 16) return "body";
  if (size >= 13 && weight >= 500) return "caption";
  return "micro";
}
...
function renderColorSection(colors: ObservedColor[], primaryHex: string): string {
...
function renderTypographySection(
  ...
  const rows = unique
    ...
    .map((s, i) => {
      ...
      const sampleText =
        role === "display" || role === "heading" ? "The quick brown fox" : "The quick brown fox jumps over the lazy dog";
...
function renderSurfacesSection(surfaces: Surface[]): string {
  const uniqueSurfaces = surfaces.filter(
    (s, i, arr) => arr.findIndex((x) => x.background === s.background && x.text === s.text) === i,
  );
...
function renderFormSection(primaryHex: string, inputRadius: string, primaryFg: string): string {
  return `<section>
...
function renderRadiusSection(radiusVals: string[]): string {
  ...
  // Always include 0px
  if (!unique.includes("0px")) unique.unshift("0px");
...
function renderElevationSection(shadows: string[], cssVars: Record<string, string>): string {
  ...
  const shadowGroups: Array<{ prefix: string; label: string }> = [
  ...
  for (const s of shadows) {
    if (s && s !== "none") realShadows.push({ label: "Observed element shadow", shadow: s });
  }
...
function renderResponsiveSection(siteUrl: string, breakpoints: BreakpointEntry[]): string {
...
// ---------------------------------------------------------------------------
export function renderNintendoHtml(
  ...
  const primaryHex =
    rawPrimaryBtn ? rgbCssToHex(rawPrimaryBtn) : (analysis.palette.primary[0] ?? "#000000");
  ...
  const dominantBg =
    analysis.surfaces.find((s) => s.role === "page_canvas")?.background ||
...
async function extractBreakpoints(runDir: string): Promise<BreakpointEntry[]> {
  ...
  const minWidthCounts = new Map<number, number>();

  try {
    const entries = await readdir(compDir);
    for (const entry of entries) {
      const domPath = join(compDir, entry, "dom.html");
      try {
        const dom = await readFile(domPath, "utf-8");
...
function breakpointDescription(px: number): string {
  if (px <= 480) return "Single-column layout; full-width touch targets; navigation collapses";
  if (px <= 768) return "Two-column grid unlocked; sidebar may appear; font scale increases";
  if (px <= 1024) return "Full navigation expanded; multi-column layouts active; container max-width starts";
  if (px <= 1280) return "Wider content area; larger hero padding; additional columns in grids";
...
// ---------------------------------------------------------------------------
export async function renderFromRunDir(runDir: string, url: string): Promise<string | null> {
  try {
