...
export const config = {
  ...
  get mongoUri() {
    ...
    if (!uri) {
      throw new Error(
        "MONGO_URI environment variable is required. Set it in your .env.local file.",
      );
    }
    return uri;
  },
  ...
  styleMdKimiCurationMaxSteps: parseInt(
    process.env.STYLEMD_KIMI_CURATION_MAX_STEPS || "80",
    10,
  ),
  ...
  get redisUrl() {
    ...
    if (!v) throw new Error("REDIS_URL environment variable is required");
    return v;
  },
